                    ...
                    if (client != null && client.Id > 0)
                    {
                        ...
                        if (client != null && client.Id > 0 && account_client != null && account_client.Id > 0)
                        {
                            var forgot_password_object = new ClientForgotPasswordTokenModel()
                            {
                                ...
                                exprire_time = DateTime.Now.AddHours(1)
                            };
                            ...
                            string uuid = Guid.NewGuid().ToString();
                             _redisService.Set("FORGOTPASSWORD_" + uuid, forgot_password_token,DateTime.Now.AddHours(1), Convert.ToInt32(configuration["Redis:Database:db_search_result"]));
        ...
        public async Task<ActionResult> ForgotChangePassword([FromBody] APIRequestGenericModel input)
        {
            try
            {
                ...
                if (input != null && input.token != null && CommonHelper.GetParamWithKey(input.token, out objParr, configuration["KEY:private_key"]))
                {
                    var request = JsonConvert.DeserializeObject<ClientChangePasswordRequestModel>(objParr[0].ToString());
                    if (request == null
                        || request.token_forgot_password == null || request.token_forgot_password.Trim() == ""
                        || request.password == null || request.password.Trim() == ""
