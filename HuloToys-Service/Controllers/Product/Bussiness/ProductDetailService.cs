        ...
        private bool UpdateProductFlashsale(ProductMongoDbModelFEResponse item, List<FlashSaleESModel> active_flashsale, List<FlashSaleProductESModel> list_item,List<GroupProductESModel> group_types)
        {
            try
            {
                ...
                if (active_flashsale != null && active_flashsale.Count > 0 && list_item != null && list_item.Count > 0)
                {
                    ...
                    LogHelper.InsertLogTelegram("UpdateProductFlashsale ["+ (item.parent_product_id != null && item.parent_product_id.Trim() != "" ? item.parent_product_id : item._id) + "]  exists_flash_sale_product ["+((exists_flash_sale_product != null && exists_flash_sale_product.flashsale_id != null) ? exists_flash_sale_product.id : "NULL") +"]");
